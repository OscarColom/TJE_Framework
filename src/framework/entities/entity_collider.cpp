...
void EntityCollider::getCollisionsWithModel(const Matrix44& m, const Vector3& target_position , std::vector<sCollisionData>& collisions, std::vector<sCollisionData>& ground_collisions) {
	...
	if (mesh->testSphereCollision(m, floor_sphere_center , sphere_ground_radius - 0.29f, collision_point, collision_normal)) {
	...
	if (mesh->testRayCollision(m, character_center_target, Vector3(0, -1, 0), collision_point, collision_normal, player_height + 0.1f )) {
